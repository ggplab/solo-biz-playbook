...
> Channels는 **고객 여정(수요측)** 축입니다. TOP(발견)·MID(관계)는 콘텐츠가 채우고, **BOTTOM(전환)은 "유료 결제 발생"으로 정의**합니다. 같은 "전환"이라도 내 생산 라인(리서치→콘텐츠→매출) 같은 공급측 표현과는 다른 축이니, 한 장으로 합치지 말고 전환 지점만 같은 정의로 맞추면 됩니다.
